...
exports.main = async (event) => {
  try {
    ...
    if (!profileIds.length) return { code: -1, message: '请选择要彻底删除的档案' };
    ...
    return { code: 0, data: { deleted: result.stats.removed } };
  } catch (err) {
    console.error('[purgeDeletedProfiles] error:', err);
    return { code: 500, message: err.message || '彻底删除档案失败' };
  }
};
